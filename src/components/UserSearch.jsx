    ...
    return (
        <div className="container" style={{ maxWidth: '800px' }}>
            <div className="card" style={{ textAlign: 'center', padding: '4rem 3rem' }}>
                ...
                <h1 style={{ marginBottom: '1.25rem', fontSize: '3.2rem', letterSpacing: '-0.05em' }}>주문번호 조회</h1>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '2rem', position: 'relative' }}>
                    <input
                        type="text"
                        placeholder="주문번호를 입력하세요"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        style={{ margin: 0, paddingLeft: '1.5rem', height: '60px', fontSize: '1.1rem' }}
                        ...
                        {result ? (
                            <div className="order-detail" style={{ borderRadius: '24px', background: 'var(--bg-glass)', padding: '2.5rem', border: '1px solid var(--border-glass)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
                                    <div>
                                        <div style={{ padding: '6px 14px', background: 'var(--accent-teal)', color: '#fff', borderRadius: '10px', display: 'inline-block', marginBottom: '1rem', fontSize: '0.85rem', fontWeight: 800 }}>
                                            조회 성공
                                        </div>
                                        <h2 style={{ fontSize: '2.2rem', letterSpacing: '-0.05em', margin: 0 }}>{result.item}</h2>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginBottom: '0.75rem' }}>
                                            <span className={`badge ${result.payment_status === '완료' ? 'badge-teal' : result.payment_status === '대기' ? 'badge-orange' : 'badge-red'}`} style={{ fontSize: '0.85rem', padding: '8px 16px' }}>
                                                {result.payment_status === '대기' ? '입금대기' : result.payment_status === '완료' ? '입금완료' : '주문취소'}
                                            </span>
                                            <span className={`badge ${result.delivery_status === '배송완료' ? 'badge-teal' : 'badge-gray'}`} style={{ fontSize: '0.85rem', padding: '8px 16px' }}>
                                                {result.delivery_status}
                                            </span>
                                        </div>
                                        <p style={{ color: 'var(--text-muted)', fontFamily: 'monospace', margin: 0, fontSize: '0.9rem' }}>#{result.id}</p>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2.5rem' }}>
                                    {[
                                        ...
                                        <div key={idx} style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
                                            <div style={{ color: 'var(--accent-teal)', padding: '0.75rem', background: 'var(--bg-card)', borderRadius: '14px', boxShadow: '0 4px 10px rgba(0,0,0,0.03)', border: '1px solid var(--border-glass)' }}>
                                                {info.icon}
                                            </div>
                                            <div>
                                                <small style={{ color: 'var(--text-muted)', display: 'block', marginBottom: '0.25rem', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{info.label}</small>
                                                <p style={{ fontWeight: 800, fontSize: '1.15rem', margin: 0 }}>{info.value}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
